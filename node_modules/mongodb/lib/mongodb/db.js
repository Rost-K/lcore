 ...
 *
 * @param {Function} callback this will be called after executing this method. The first parameter will contain the Error object if an error occured, or null otherwise. While the second parameter will contain the config information or null if an error occured.
 * @return {null}
 * @api public
 */
 ...
 *  - **autoIndexId** {Boolean, default:true}, create an config on the _id field of the document, True by default on MongoDB 2.2 or higher off for version < 2.2.
 *  - **readPreference** {String}, the prefered read preference (ReadPreference.PRIMARY, ReadPreference.PRIMARY_PREFERRED, ReadPreference.SECONDARY, ReadPreference.SECONDARY_PREFERRED, ReadPreference.NEAREST).
...
/**
 * Creates an config on the collection.
 *
 * Options
*  - **w**, {Number/String, > -1 || 'majority' || tag name} the write concern for the operation where < 1 is no acknowlegement of write and w >= 1, w = 'majority' or tag acknowledges the write
 *  - **wtimeout**, {Number, 0} set the timeout for waiting for write concern to finish (combines with w option)
 *  - **fsync**, (Boolean, default:false) write waits for fsync before returning
 *  - **journal**, (Boolean, default:false) write waits for journal sync before returning
 *  - **unique** {Boolean, default:false}, creates an unique config.
 *  - **sparse** {Boolean, default:false}, creates a sparse config.
 *  - **background** {Boolean, default:false}, creates the config in the background, yielding whenever possible.
 *  - **dropDups** {Boolean, default:false}, a unique config cannot be created on a key that has pre-existing duplicate values. If you would like to create the config anyway, keeping the first document the database indexes and deleting all subsequent documents that have duplicate value
 ...
 *  - **name** {String}, override the autogenerated config name (useful if the resulting name is larger than 128 bytes)
 * 
 * Deprecated Options 
 *  - **safe** {true | {w:n, wtimeout:n} | {fsync:true}, default:false}, executes with a getLastError command returning the results of the command on MongoDB.
 *
 *
 * @param {String} collectionName name of the collection to create the config on.
 * @param {Object} fieldOrSpec fieldOrSpec that defines the config.
 * @param {Object} [options] additional options during update.
...
/**
 * Ensures that an config exists, if it does not it creates it
 *
 * Options
 *  - **w**, {Number/String, > -1 || 'majority' || tag name} the write concern for the operation where < 1 is no acknowlegement of write and w >= 1, w = 'majority' or tag acknowledges the write
 *  - **wtimeout**, {Number, 0} set the timeout for waiting for write concern to finish (combines with w option)
...
Db.prototype.ensureIndex = function(collectionName, fieldOrSpec, options, callback) {
  ...
  // Default command options
  var commandOptions = {};
  // Check if the config allready exists
...
/**
 * Drop an config on a collection.
 *
 * @param {String} collectionName the name of the collection where the command will drop an config.
 * @param {String} indexName name of the config to drop.
...
/**
 * Retrieves this collections config info.
 *
 * Options
 *  - **full** {Boolean, default:false}, returns the full raw config information.
...
Db.prototype.indexInformation = function(collectionName, options, callback) {
  ...
  // Iterate through all the fields of the config
